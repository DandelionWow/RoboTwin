...
robotwin_root = os.path.dirname(parent_directory)
...
DEFAULT_DATASET_ROOT = os.path.abspath(
    os.path.join(parent_directory, "../../..", "database", "datasets", "robotwin_480_640")
)


def resolve_cli_path(path):
    """Resolve CLI paths relative to the directory where the command was invoked."""
...
def read_seed_list_from_traj_dir(traj_data_dir):
    """Read seed.txt beside a source _traj_data directory."""
    source_save_path = os.path.dirname(os.path.abspath(traj_data_dir))
    return read_seed_list(source_save_path)


def list_traj_episode_indices(traj_data_dir):
    """Return sorted episode indices available in a source _traj_data directory."""
    if not os.path.isdir(traj_data_dir):
        raise SystemExit(f"--traj-data-dir does not exist or is not a directory: {traj_data_dir}")

    indices = []
    for filename in os.listdir(traj_data_dir):
        if not (filename.startswith("episode") and filename.endswith(".pkl")):
            continue
        episode_text = filename[len("episode") : -len(".pkl")]
        try:
            indices.append(int(episode_text))
        except ValueError:
            continue
    if not indices:
        raise SystemExit(f"No episode*.pkl files found under --traj-data-dir: {traj_data_dir}")
    return sorted(indices)


def load_traj_data_from_dir(traj_data_dir, episode_idx):
    """Load a trajectory pkl from an explicit _traj_data directory."""
    file_path = os.path.join(traj_data_dir, f"episode{episode_idx}.pkl")
...
def list_dataset_source_dirs(task_root):
    """Return source config dirs that contain seed.txt and _traj_data."""
    source_dirs = []
    for config_name in sorted(os.listdir(task_root)):
        candidate = os.path.join(task_root, config_name)
        if os.path.isfile(os.path.join(candidate, "seed.txt")) and os.path.isdir(os.path.join(candidate, "_traj_data")):
            source_dirs.append(candidate)
    return source_dirs


def find_dataset_task_dir(dataset_root, task_name, source_config=None):
    """Resolve `<dataset_root>/<task>/<config>` for replay source data."""
    dataset_root = os.path.abspath(dataset_root)
    task_root = os.path.join(dataset_root, task_name)
    if not os.path.isdir(task_root):
        raise SystemExit(f"Cannot find task {task_name} under dataset root: {dataset_root}")

    if source_config:
        expected = os.path.join(task_root, source_config)
        if os.path.isfile(os.path.join(expected, "seed.txt")) and os.path.isdir(os.path.join(expected, "_traj_data")):
            return expected
        raise SystemExit(f"Cannot find seed.txt and _traj_data for task {task_name} under {expected}")

    candidates = list_dataset_source_dirs(task_root)
    if len(candidates) == 1:
        return candidates[0]
    if candidates:
        candidate_text = ", ".join(os.path.basename(path) for path in candidates[:10])
        raise SystemExit(
            f"Cannot choose replay source config for task {task_name}. "
            f"Pass --source-config. Candidates: {candidate_text}"
        )
    raise SystemExit(f"Cannot find any source config with seed.txt and _traj_data under {task_root}")


def discover_dataset_tasks(dataset_root, source_config=None):
    """List tasks that contain source replay data for the requested config."""
    dataset_root = os.path.abspath(dataset_root)
    if not os.path.isdir(dataset_root):
        raise SystemExit(f"Dataset root does not exist: {dataset_root}")

    task_names = []
    for task_name in sorted(os.listdir(dataset_root)):
        task_root = os.path.join(dataset_root, task_name)
        if not os.path.isdir(task_root):
            continue

        if source_config:
            source_dir = os.path.join(task_root, source_config)
            has_source = os.path.isfile(os.path.join(source_dir, "seed.txt")) and os.path.isdir(os.path.join(source_dir, "_traj_data"))
        else:
            has_source = len(list_dataset_source_dirs(task_root)) == 1
    ...
    if not task_names:
        if source_config:
            raise SystemExit(f"Cannot find any task with {source_config}/seed.txt and _traj_data under {dataset_root}")
        raise SystemExit(f"Cannot find any task with a unique source config containing seed.txt and _traj_data under {dataset_root}")
    return task_names


def parse_episodes(episodes_text):
    """Parse episode selections like `0-10` into inclusive source episode ids."""
    if episodes_text is None:
        return None

    episode_indices = []
    seen = set()
    for raw_part in str(episodes_text).replace(" ", "").split(","):
        if not raw_part:
            continue
        if "-" in raw_part:
            ...
            if start_episode < 0 or end_episode < start_episode:
                raise SystemExit("--episodes ranges require START >= 0 and END >= START")
            part_indices = range(start_episode, end_episode + 1)
        else:
            episode_idx = int(raw_part)
            if episode_idx < 0:
                raise SystemExit("--episodes values must be >= 0")
            part_indices = [episode_idx]

        for episode_idx in part_indices:
            if episode_idx not in seen:
                episode_indices.append(episode_idx)
                seen.add(episode_idx)

    if not episode_indices:
        raise SystemExit("--episodes did not contain any episode index")
    return episode_indices


def select_traj_episode_indices(traj_data_dir, episodes=None):
    """Select source trajectory episode indices."""
    available_indices = list_traj_episode_indices(traj_data_dir)
    if episodes is None:
        return available_indices

    available_set = set(available_indices)
    selected_indices = list(episodes)
    missing_indices = [idx for idx in selected_indices if idx not in available_set]
    if missing_indices:
        missing_text = ", ".join(str(idx) for idx in missing_indices[:20])
        if len(missing_indices) > 20:
            missing_text += ", ..."
        raise SystemExit(f"Missing trajectory episode(s) in {traj_data_dir}: {missing_text}")
    return selected_indices


def select_seeds_for_traj_episodes(source_save_path, traj_episode_indices, override_seeds=None):
    """Return replay seeds aligned with source trajectory episode indices."""
    if override_seeds:
        if len(override_seeds) != len(traj_episode_indices):
            raise SystemExit(
                f"Received {len(override_seeds)} explicit seed(s) for "
                f"{len(traj_episode_indices)} source trajectory episode(s)."
            )
        return list(override_seeds)

    source_seeds = read_seed_list(source_save_path)
    if not source_seeds:
        raise SystemExit(
            f"No seeds were provided and no seed.txt was found under source dataset: {source_save_path}"
        )

    missing_seed_indices = [idx for idx in traj_episode_indices if idx >= len(source_seeds)]
    if missing_seed_indices:
        missing_text = ", ".join(str(idx) for idx in missing_seed_indices[:20])
        if len(missing_seed_indices) > 20:
            missing_text += ", ..."
        raise SystemExit(
            f"{os.path.join(source_save_path, 'seed.txt')} has {len(source_seeds)} seed(s), "
            f"cannot replay source episode(s): {missing_text}"
        )
    return [source_seeds[idx] for idx in traj_episode_indices]
...
def collect_hdf5_for_seed(TASK_ENV, args, episode_idx, seed, clear_cache, traj_data_dir=None, traj_episode_idx=None):
    ...
    Args:
        ...
        traj_data_dir: 可选的外部 `_traj_data` 目录。提供后直接从该目录
            读取 `episode<traj_episode_idx>.pkl`，跳过当前输出目录中的轨迹。
        traj_episode_idx: 外部轨迹文件的 episode 下标；未提供时使用
            `episode_idx`。
    ...
    try:
        ...
        source_episode_idx = episode_idx if traj_episode_idx is None else traj_episode_idx
        if traj_data_dir:
            traj_data = load_traj_data_from_dir(traj_data_dir, source_episode_idx) # 从指定 _traj_data 加载轨迹
        else:
            traj_data = TASK_ENV.load_tran_data(source_episode_idx) # 加载当前数据集中的轨迹数据
...
def collect_data_from_given_seeds(TASK_ENV, args, seeds, start_episode=None, overwrite=False):
    ...
    traj_data_dir = args.get("traj_data_dir")
    traj_episode_indices = args.get("traj_episode_indices") or []

    print("\033[93m" + "[Start Data Collection From Given Seeds]" + "\033[0m")
    print(f"Start episode: {start_episode}, Seeds: {seeds}")
    if traj_data_dir:
        print(f"Replay from traj data: {traj_data_dir}")

    for seed_offset, seed in enumerate(seeds):
        traj_episode_idx = None
        if traj_data_dir:
            if seed_offset >= len(traj_episode_indices):
                raise SystemExit(
                    f"Not enough trajectory files in --traj-data-dir for {len(seeds)} seed(s); "
                    f"found {len(traj_episode_indices)}."
                )
            traj_episode_idx = traj_episode_indices[seed_offset]
        ...
        try:
            if traj_data_dir:
                print(
                    f"\033[34mTask name: {args['task_name']}, episode: {episode_idx}, "
                    f"seed: {seed}, source traj episode: {traj_episode_idx}\033[0m"
                )
            else:
            ...
            collect_hdf5_for_seed(
                ...
                traj_episode_idx=traj_episode_idx,
            ) # replay 轨迹并保存数据
            ...
            print("Error: ", e)
            print(" -------------")
            if not traj_data_dir and not getattr(e, "failure_data_saved", False):
...
def collect_data_from_seed_list(TASK_ENV, args, seed_list):
    ...
    traj_episode_indices = args.get("traj_episode_indices") or []
    if traj_data_dir:
        max_episode_num = min(args["episode_num"], len(seed_list), len(traj_episode_indices))
        print(f"Replay from traj data: {traj_data_dir}")
    else:
        max_episode_num = args["episode_num"]

    for episode_idx in range(st_idx, max_episode_num):
        print(f"\033[34mTask name: {args['task_name']}\033[0m")
        traj_episode_idx = traj_episode_indices[episode_idx] if traj_data_dir else None
...
def main(
    ...
    Args:
        ...
        traj_data_dir: 指定外部 `_traj_data` 目录时，跳过预规划并直接 replay
            其中的 `episode*.pkl`。
        dataset_root: 指定源数据集根目录时，从
            `<dataset_root>/<task_name>/<source_config>/_traj_data`
            读取轨迹；未指定 `source_config` 时自动发现唯一源目录。
        source_config: 可选源轨迹配置目录名。只在 task 下有多个源目录时需要。
        episodes: 可选 source episode 下标列表；例如 `0-10` 会被解析成
            `[0, 1, ..., 10]`。
    ...
    if dataset_root is None and traj_data_dir is None and episodes is not None:
        dataset_root = DEFAULT_DATASET_ROOT
    dataset_root = resolve_cli_path(dataset_root)
    traj_data_dir = resolve_cli_path(traj_data_dir)

    traj_episode_indices = []
    if dataset_root is not None:
        if traj_data_dir is not None:
            raise SystemExit("Use either --dataset-root or --traj-data-dir, not both.")
        source_save_path = find_dataset_task_dir(dataset_root, task_name, source_config)
        traj_data_dir = os.path.join(source_save_path, "_traj_data")
        traj_episode_indices = select_traj_episode_indices(traj_data_dir, episodes)
        seeds = select_seeds_for_traj_episodes(source_save_path, traj_episode_indices, override_seeds=seeds)
        print(f"Dataset replay source: {source_save_path}")
        print(
            f"Source episodes: {traj_episode_indices[0]}-{traj_episode_indices[-1]} "
            f"({len(traj_episode_indices)} episode(s))"
        )
    elif traj_data_dir is not None:
        traj_data_dir = os.path.abspath(traj_data_dir)
        source_save_path = os.path.dirname(traj_data_dir)
        traj_episode_indices = select_traj_episode_indices(traj_data_dir, episodes)
        seeds = select_seeds_for_traj_episodes(source_save_path, traj_episode_indices, override_seeds=seeds)
    ...
    if episode_num is not None:
        ...
        args["episode_num"] = min(len(seeds), len(traj_episode_indices))

    if traj_data_dir is not None:
        replay_num = min(args["episode_num"], len(seeds), len(traj_episode_indices))
        seeds = list(seeds)[:replay_num]
        traj_episode_indices = traj_episode_indices[:replay_num]
        args["episode_num"] = replay_num
    ...
    args["traj_data_dir"] = traj_data_dir
    args["traj_episode_indices"] = traj_episode_indices
...
def run(TASK_ENV, args):
    ...
    if args.get("given_seeds"): # 显式指定 seed 模式，直接采集数据；traj_data_dir 模式下跳过预规划
    ...
    elif args.get("traj_data_dir"):
        raise SystemExit("Replay from --traj-data-dir requires seeds or a seed.txt beside the _traj_data directory.")
...
def collect_dataset_replay(
    ...
    """Replay one or more tasks from a dataset source."""
    dataset_root = resolve_cli_path(dataset_root) or DEFAULT_DATASET_ROOT
    task_names = list(tasks or discover_dataset_tasks(dataset_root, source_config))

    print(f"Dataset root: {dataset_root}")
    print(f"Task config: {task_config}")
    print(f"Source config: {source_config or 'auto'}")
    print(f"Tasks: {', '.join(task_names)}")
    if episodes is not None:
        print(f"Episodes: {episodes[0]}-{episodes[-1]} ({len(episodes)} episode(s))")

    for task_idx, task_name in enumerate(task_names, start=1):
        print(f"\033[95m[Dataset Replay Task {task_idx}/{len(task_names)}] {task_name}\033[0m")
        main(
            task_name=task_name,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "positional",
        nargs="*",
        help="Legacy form: task_name task_config [seeds...]",
    )
    parser.add_argument("--task-config", type=str, default=None, help="Task config to replay, e.g. demo_large_d435_replay")
    parser.add_argument("--seed", dest="seed_options", action="append", default=[], help="Seed value; can be used multiple times")
    parser.add_argument("--tasks", nargs="+", default=None, help="Task names to replay; defaults to all tasks in dataset root")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Replay trajectories directly from this _traj_data directory and skip pre-motion planning",
    )
    parser.add_argument(
        "--dataset-root",
        type=str,
        default=None,
        help=f"Replay source dataset root. Defaults to {DEFAULT_DATASET_ROOT}",
    )
    parser.add_argument(
        "--source-config",
        type=str,
        default=None,
        help="Optional source dataset config directory. Defaults to auto-discovering the unique source under each task",
    )
    parser.add_argument("--source-task-config", type=str, default=None, help=SUPPRESS)
    parser.add_argument(
        "--episodes",
        type=str,
        default=None,
        help="Inclusive source episodes, e.g. 0-10 means 11 samples from episode0 through episode10",
    )
    parser.add_argument("--episode-range", nargs=2, type=int, default=None, help=SUPPRESS)
    parser = parser.parse_args()
    episodes = parse_episodes(parser.episodes)
    if parser.episode_range is not None:
        if episodes is not None:
            raise SystemExit("Use --episodes instead of combining it with --episode-range")
        start_episode, end_episode = parser.episode_range
        episodes = parse_episodes(f"{start_episode}-{end_episode}")

    if parser.source_task_config is not None:
        raise SystemExit("--source-task-config is deprecated. Use --source-config if the replay source is ambiguous.")

    if parser.task_config is not None:
        if parser.positional:
            raise SystemExit("With --task-config, specify tasks via --tasks instead of positional arguments.")
        task_config = parser.task_config
        if parser.traj_data_dir is not None and not parser.tasks:
            raise SystemExit("--traj-data-dir with the new CLI form requires exactly one task in --tasks")
        if parser.traj_data_dir is not None and len(parser.tasks or []) != 1:
            raise SystemExit("--traj-data-dir can only be used with one --tasks value")

        if parser.traj_data_dir is not None:
            seeds = parse_seed_values(parser.seed_options)
            main(
                task_name=parser.tasks[0],
                task_config=task_config,
                seeds=seeds,
                start_episode=parser.start_episode,
                overwrite=parser.overwrite,
                episode_num=parser.episode_num,
                traj_data_dir=parser.traj_data_dir,
                source_config=parser.source_config,
                episodes=episodes,
            )
        else:
            collect_dataset_replay(
                task_config=task_config,
                tasks=parser.tasks,
                dataset_root=parser.dataset_root,
                source_config=parser.source_config,
                episodes=episodes,
                start_episode=parser.start_episode,
                overwrite=parser.overwrite,
    ...
    elif len(parser.positional) == 1:
        task_config = parser.positional[0]
        if parser.traj_data_dir is not None:
            if not parser.tasks or len(parser.tasks) != 1:
                raise SystemExit("--traj-data-dir requires exactly one task in --tasks")
            seeds = parse_seed_values(parser.seed_options)
            main(
                task_name=parser.tasks[0],
                task_config=task_config,
    ...
    elif len(parser.positional) >= 2:
        if parser.tasks:
            raise SystemExit("With --tasks, use: collect_data_replay.py --task-config CONFIG --tasks task_a task_b")
        task_name = parser.positional[0]
        task_config = parser.positional[1]
        seeds = parse_seed_values(parser.seed_options + parser.positional[2:])
        main(
            task_name=task_name,
            task_config=task_config,
            seeds=seeds,
        ...
        raise SystemExit("Missing --task-config. Example: collect_data_replay.py --task-config demo_large_d435_replay --episodes 0-10")
